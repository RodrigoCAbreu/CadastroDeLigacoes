...
function remover( id ) {
			if (confirm("Remove o Exame com id " + id)) {
				$('#fExame').empty();
				$('#fExame').append('<input type="hidden" name="idExame" value="' + id + '"/>');
				$('#fExame').append('<input type="hidden" name="cmdE" value="remover"/>');
				$('#fExame').submit();
			}
		}
		...
		function editar( id ) {
			$('#fExame').empty();
			$('#fExame').append('<input type="hidden" name="idExame" value="' + id + '"/>');
			$('#fExame').append('<input type="hidden" name="cmdE" value="editar"/>');
			$('#fExame').submit();
		}
</script>
</head>
...
<%
	...
	if (exameAtual == null) {
		...
		session.setAttribute("EXAME_ATUAL", null);
	}
